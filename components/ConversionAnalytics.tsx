      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white rounded-[3rem] border border-slate-200 p-12 shadow-sm flex flex-col items-center justify-center text-center">
          ...
          <p className="text-4xl font-black text-blue-600 tracking-tighter mb-4">
            {trendData.find(d => d.year === activeYear)?.conversionRate.toFixed(1) || '0.0'}%
          </p>
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-relaxed">
            Consolidated enquiry to registration <br/> velocity for the current session.
          </p>
          <div className="mt-8 pt-8 border-t border-slate-100 w-full flex justify-between px-4">
             <div className="text-left">
                ...
                <p className="text-lg font-black text-slate-900">{trendData.find(d => d.year === activeYear)?.enquiries.toLocaleString() || '0'}</p>
             </div>
             <div className="text-right">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Registrations</p>
                <p className="text-lg font-black text-slate-900">{trendData.find(d => d.year === activeYear)?.registration.toLocaleString() || '0'}</p>
             </div>
          </div>
        </div>
      </div>
